...
export function Testimonials() {
  ...
  const { data: managedTestimonials } = useCmsSection<any[]>('testimonials', import.meta.env.DEV ? fallbackTestimonials : []);
  const testimonials = managedTestimonials.length
    ...
    : (import.meta.env.DEV ? fallbackTestimonials : []);
      ...
      <div className="max-w-[1280px] mx-auto px-4 md:px-8">
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          {testimonials.length === 0 ? (
            <Card className="bg-primary p-8 md:p-12 text-center border-border-gold/30 text-text-muted">
              {t('سيتم نشر شهادات العملاء المعتمدة قريباً', 'Approved client testimonials will be published soon')}
            </Card>
          ) : (
            <>
