...
export function MarketsPreview() {
  ...
  const markets = [
    { nameAr: 'تاسي (السعودية)', nameEn: 'TASI (Saudi Arabia)', symbol: 'TASI', price: '11,940.50', change: '+0.8%', isUp: true },
    { nameAr: 'سوق دبي المالي', nameEn: 'DFM (Dubai)', symbol: 'DFMGI', price: '4,120.30', change: '+1.2%', isUp: true },
    { nameAr: 'ناسداك (أمريكا)', nameEn: 'NASDAQ (US)', symbol: 'IXIC', price: '16,240.80', change: '-0.4%', isUp: false },
    { nameAr: 'الذهب', nameEn: 'Gold', symbol: 'XAU/USD', price: '$2,345.10', change: '+0.2%', isUp: true },
    { nameAr: 'بيتكوين', nameEn: 'Bitcoin', symbol: 'BTC/USD', price: '$68,450.00', change: '+3.5%', isUp: true },
    { nameAr: 'برنت الخام', nameEn: 'Brent Crude', symbol: 'BZ', price: '$82.30', change: '-1.1%', isUp: false },
  ];
    ...
    <section className="py-24 bg-secondary dark:bg-[#13132A] relative overflow-hidden">
      <div className="max-w-[1280px] mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {markets.map((m, i) => (
            <Card key={i} variant="interactive" className="p-6 flex flex-col justify-between h-full bg-primary group animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${i*100}ms` }}>
              <div className="flex justify-between items-start mb-6">
                <div>
                  <div className="font-bold text-lg text-text-primary mb-1 group-hover:text-gold-deep transition-colors">{lang === 'ar' ? m.nameAr : m.nameEn}</div>
                  <div className="text-xs font-mono text-text-muted">{m.symbol}</div>
                </div>
                <div className={cn("flex items-center gap-1 rounded-sm px-2 py-1 text-[12px] font-mono font-bold", m.isUp ? "bg-success-light text-success" : "bg-error-light text-error")}>
